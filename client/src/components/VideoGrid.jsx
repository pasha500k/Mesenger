...
const VideoGrid = ({ localStream, remoteStreams, callActive }) => {
  if (!callActive) {
    return (
  ...
  const streams = [];
  if (localStream) {
    streams.push({ id: 'local', stream: localStream, label: 'Вы', muted: true });
  }
  ...
  return (
    <div className="grid gap-4" style={{ gridTemplateColumns: streams.length > 1 ? 'repeat(auto-fit, minmax(220px, 1fr))' : '1fr' }}>
      {streams.map((item) => (
        <VideoTile key={item.id} stream={item.stream} label={item.label} muted={item.muted} />
      ))}
      {streams.length === 0 && (
        <div className="h-full rounded-3xl border border-dashed border-white/20 flex items-center justify-center text-slate-400 bg-white/5">
